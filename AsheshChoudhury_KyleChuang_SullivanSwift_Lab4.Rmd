...
author: "Ashesh Choudhury, Kyle Chaung, and Sullivan Swift"
date: "Fall 2018"
output:
...
(Question 2) Three states, 5, 44 and 10, significantly increased in population while other states were relatively flat. This suggests using a population-normalized fatality measure, such as `totfatrte`, for examining traffic laws' impact on driving fatalities.
...
We performed cross-sectional t-tests for 1999 and 2000 where the percentages are closer to even for `sbprim` and `sbsecon`. For both t-tests, we fail to reject $H_0$ and there does not appear to be any contemporaneous differences between seatbelt laws. Despite the decrease of `sbsecon` mix from 1995 to 2004, the fatalities from 1995 to 2004 is similar. There is unlikely to be a lagged impact of seatbelt law differences. Finally, we also note that state 30 does not have seatbelt laws throughout the period if further analysis on specifics of the seatbelt law is required.
...
Vehicle miles per capital may be secondarily affected by traffic laws, such as graduated license laws. Preliminarily, fatalities appear to decrease as it increases. This relationship is counterintuitive and is likely to be a trending effect through time. Other independent variables likely have a stronger impact than the number of miles driven on total fatalities rates.
...
Fatalities do not appear to be affected by unemployment rates at all. In fact, the pattern appears somewhat random. We expect this variable to have a $\beta_{unem}$ close to 0 in regressions. If we do include the variable, we may want to consider logging it to spread out the values and improve the regression. It does not appear heteroskedastic.

```{r}
plotmix(data,'unem','Unemployment')
```

Percent of peopl ages 14-24 is left skewed and a log transformation will be performed. It appears that percent 14-24 is negatively correlated with fatalities. This may potentially be an artifact of the general trend in both population and fatalities.
...
(Question 3) We will now expand the previous regression with additional regressors - `bac08`, `bac10`, `perse`, `sbprim`, `sbsecon`, `sl70plus`, `gld`, `perc14_24`, `unem` and `vehicmilespc`. `perc14-24` is logged since it is very left skewed and `unem` is also logged. Logging the values better approximates a normal distribution, thus leading to a better regression results. `vehicmilepc` does not appear to require transformations as it appears more normally/uniformly distributed. The rest of variables are binary and no transformations are performed. BAC and speed limit variables are not binarized and no interactions are implemented to limit the report length.
...
Additionally, we can do a pool test to check if the coefficients are consistent vs a fixed effects model. The null hypothesis, $H_0$, is rejected showing that the estimates are not consistent.
...
The final model is
\[ \begin{aligned}
y_{it}-\bar{y}_i=& -1.95 (x_{(bac08)it}-\bar{x}_{bac08}) +  -1.56 (x_{(bac10)it}-\bar{x}_{bac10}) -1.56 (x_{(per se)it}-\bar{x}_{per se}) \\
...
&+ 14.66(x_{(log(per14_24))it}-\bar{x}_{log(perc14_24)})+-0.59(x_{(log(unem))it}-\bar{x}_{log(unem)})\\
&+0.0003(x_{(vehicmilespc)it}-\bar{x}_{vehicmilespc})
\end{aligned}
\]
